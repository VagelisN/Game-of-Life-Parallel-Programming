...
int main(int argc, char  *argv[])
{
	...
	int rank, temprank, numprocs, j, k, i, provided;;
	MPI_Datatype column;
	...
	if(provided!= MPI_THREAD_FUNNELED){printf("MPI_THREAD_FUNNELED thread support required\n");exit(1);}
	...
	if(cart_size_x==-1){if(rank==0){fprintf(stderr, "Number of processes given is not acceptable!\n");exit(1);}}
...
{
	#pragma omp for collapse(2)//give a number of iterations to each thread 
...
#pragma omp parallel private(neighbours,i,j)
{
	#pragma omp sections//each thread is given to calculate one column or one row (north-east-west-south)
	{
			#pragma omp section
			{
	...
	local_finish = MPI_Wtime();//each process calculates its elapsed time
